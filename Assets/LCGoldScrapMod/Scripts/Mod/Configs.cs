...
public class Configs
{
    ...
    public static void LoadAndDisplayConfigs()
    {
        ...
        {
            if (Plugin.goldScrapSpawnEnabled) Logger.LogWarning($"Config [Multiplier Price] is set to an invalid value of {priceMultiplier.Value}. Resetting to default value.");
            priceMultiplier.Value = 1.0f;
        }
        else
        {
            if (Plugin.goldScrapSpawnEnabled) Logger.LogInfo($"Config [Multiplier Price] is set to a value of {priceMultiplier.Value}");
        }
    ...
    public static void SetCustomGoldStorePrices(float multiplierHost)
    {
        hostPriceMultiplier = multiplierHost;
        foreach (ItemData item in StoreAndTerminal.allGoldStoreItemData)
        {
            if (item.storeDefaultPrice == -1)
            {
                continue;
            }
            ...
            {
                for (int i = 0; i < item.storeTerminalNodes.Length; i++)
                {
                    if (item.storeTerminalNodes[i] == null)
                    {
                        continue;
                    }
                    item.localStorePrice = thisItemPrice;
